...
async function goVRViewer(camera) {
  ...
  // Destroy lobby — releases the WebGL context on vr-canvas. If presenting
  // in XR this also ends the session (dropping back to the flat page) —
  // the viewer has its own ENTER XR button to re-enter. Handing the live
  // session + renderer off directly was tried and caused a black render,
  // so each screen gets its own fresh renderer instead.
  _vrLobby.destroy();
  _vrLobby = null;
